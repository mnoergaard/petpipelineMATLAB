function [Err,Par,RoiName]=CreateMRTM2(varargin)
%
% [Err,Par,RoiName]=CreateMRTM2([tacFN,[StartStopTime,[SIFfn,Isotope]]])
%
%  Function that creates a txt file that can be imported into database with
%  the MRTM2 estimate. If missing frames in the middle of time series then mean
...
%
[TimeTAC,Weights,RefTAC,RoiTAC,Name,Dummy,DocTxt]=KinmodEvaluateInpArg('CreateMRTM2',varargin{:});
RoiName=Name.Roi;
...
%
% Estimate k2' using MRTM
%
[Err_mrtm,Par_mrtm,yest_mrtm,Par_cov_mrtm,LogLike_mrtm,AIC_mrtm,MSE_mrtm,FPE_mrtm]=...
    mrtm_estm(TimeTAC,Weights,RefTAC,HighBindTAC,NameHighBind);
k2p=Par_mrtm{1}(2);
k2p_cov=Par_cov_mrtm{1}(2,2);
%
% Estimate parameters
%
[Err,Par,yest,Par_cov,LogLike,AIC,MSE,FPE]=mrtm2_estm(TimeTAC,Weights,RefTAC,RoiTAC,Name,k2p);
%
...
%
fprintf('\n\nData for MRTM2 model\n\n');
fprintf('   Data from %s\n\n',Name.Label');
for i=1:size(RoiTAC,2)
    fprintf('   MRTM2: %s: k2''=%5.3f, R1=%5.3f, BPnd=%5.3f, WeightErr: %6.2e, MSE: %6.2e, FPE: %6.2e, LogLikelihood: %6.3e, AIC: %6.2e\n',...
        Name.Roi{i},k2p,Par{i},Err(i),MSE(i),FPE(i),LogLike(i),AIC(i));
end
%
[pn,fn,ext]=fileparts(Name.Label);
print('-djpeg','-r300',[fn(1:end-4) 'km-mrtm2']);
%
% Save data to file
%
[pn,fn,ext]=fileparts(Name.Label);
pid=fopen([fn(1:end-4) 'km-mrtm2.tsv'],'w');
if pid==-1
    warning('MRTM2: not possible to open file for saving MRTM2 results - probably write protected directory\n\n');
else
    %
    ...
    % Write data
    %
    fprintf(pid,'MRTM2\tk2''\tR1\tBPnd\tMSE\tLogLikelihood\tAIC\t%%Cov k2''\t%%Cov R1\t%%Cov BPnd\n');
    for i=1:size(RoiTAC,2)
    ...
    function MakePlot(DataTAC,Name,ROIs)
        %
        fig_h=findobj('type','figure','tag','CreateMRTM2fig');
        if ROIs==-1
            New=1;
            delete(fig_h);
        else
        ...
        if isempty(fig_h)||New==1
            h=figure;
            set(h,'tag','CreateMRTM2fig');
            ud.Data=DataTAC;
            ud.Name=Name;
            set(h,'UserData',ud);
            mh=uimenu(h,'Label','MRTM2');
            seh1=uimenu(mh,'Label','Select ROIs','Accelerator','S','CallBack','CreateMRTM2(''ShowROIs'');');
        else
        ...
        h=title(sprintf('MRTM2 fit: %s (%s)' ,Name.Label,datestr(clock,0)));
