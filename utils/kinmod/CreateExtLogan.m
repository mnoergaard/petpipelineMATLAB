function [Err,Par,RoiName]=CreateExtLogan(varargin)
%
% [Err,Par,RoiName]=CreateExtLogan([tacFN,[StartStopTime,[SIFfn,Isotope]]])
%
%  Function that creates a txt file that can be imported into database with
%  the Logan model estimate, The logan method is implemented with a
%  refwerence region, like in the Logan paper 1996, JCBFM 16:834-40
%
%  Further, in this implementation it has been implemented to include a
...
%
[TimeTAC,Weights,RefTAC,RoiTAC,Name,TimeChlng,DocTxt]=KinmodEvaluateInpArg('CreateExtLogan',varargin{:});
RoiName=Name.Roi;
%
% Estimate parameters
%
[Err,Par,yest,Par_cov,LogLike,AIC,MSE,FPE]=logan_ext_estm(TimeTAC,Weights,RefTAC,RoiTAC,Name,TimeChlng);
%
...
%
fprintf('\n\nData for Logan model\n\n');
fprintf('   Data from %s\n\n',Name.Label');
for i=1:size(RoiTAC,2)
    fprintf('   Logan: %s: Kappa2_0=%5.3f, Vt_0=%5.3f, Kappa2_1=%5.3f, Vt_1=%5.3f, WeightErr: %6.2e, MSE: %6.2e, FPE: %6.2e, LogLikelihood: %6.3e, AIC: %6.2e\n',...
...
%
[pn,fn,ext]=fileparts(Name.Label);
pid=fopen([fn '.logan'],'w');
if pid==-1
    warning('Logan: not possible to open file for saving Logan results - probably write protected directory\n\n');
else
    %
    ...
    % Write data
    %
    fprintf(pid,'Logan\tKappa2_0\tVt_0\tKappa2_1\tVt_1\tMSE\tLogLikelihood\tAIC\t%%Cov Kappa2_0\t%%Cov Vt_0\t%%Cov Kappa2_1\t%%Cov Vt_1\n');
    for i=1:size(RoiTAC,2)
