function [Err,Par,RoiName]=CreateLogan(varargin)
%
% [Err,Par,RoiName]=CreateLogan([tacFN,[StartStopTime,[SIFfn,Isotope]]])
%
...
%
[TimeTAC,Weights,RefTAC,RoiTAC,Name,Dummy,DocTxt]=KinmodEvaluateInpArg('CreateLogan',varargin{:});
RoiName=Name.Roi;
%
% Estimate parameters
%
[Err,Par,yest,Par_cov,LogLike,AIC,MSE,FPE]=logan_estm(TimeTAC,Weights,RefTAC,RoiTAC,Name);
%
...
for i=1:size(RoiTAC,2)
    fprintf('   Logan: %s: Kappa2=%5.3f, Vt=%5.3f, WeightErr: %6.2e, MSE: %6.2e, FPE: %6.2e, LogLikelihood: %6.3e, AIC: %6.2e\n',...
...
if pid==-1
    ...
    % Write data
    %
    fprintf(pid,'Logan\tKappa2\tVt\tMSE\tLogLikelihood\tAIC\t%%Cov Kappa2\t%%Cov Vt\n');
    for i=1:size(RoiTAC,2)
        fprintf(pid,'%s\t%e\t%e\t%e\t%e\t%e\t%e\t%e\n',...
