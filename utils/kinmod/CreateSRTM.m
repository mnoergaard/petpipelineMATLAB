function [Err,Par,RoiName]=CreateSRTM(varargin)
%
% [Err,Par,RoiName]=CreateSRTM([tacFN,[StartStopTime,[SIFfn,Isotope]]])
%
...
%
[TimeTAC,Weights,RefTAC,RoiTAC,Name,dummy1,DocTxt]=KinmodEvaluateInpArg('CreateSRTM',varargin{:});
RoiName=Name.Roi;
%
% Estimate parameters
%
...
for i=1:size(RoiTAC,2)
    fprintf('   SRTM: %s: R1=%5.3f, k2=%5.3f, BPnd=%5.3f, WeightErr: %6.2e, MSE: %6.2e, FPE: %6.2e, LogLikelihood: %6.3e, AIC: %6.2e\n',...
        Name.Roi{i},Par{i},Err(i),MSE(i),FPE(i),LogLike(i),AIC(i));
end
%
fig_h=findobj('type','figure','tag','CreateSRTMfig');
[pn,fn,ext]=fileparts(Name.Label);
figure(fig_h);
print('-djpeg','-r300',[fn(1:end-4) 'km-srtm']);
%
% Save data to file
%
[pn,fn,ext]=fileparts(Name.Label);
pid=fopen([fn(1:end-4) 'km-srtm.tsv'],'w');
if pid==-1
    warning('SRTM: not possible to open file for saving SRTM results - probably write protected directory\n\n');
else
    %
    ...
    % Write data
    %
    fprintf(pid,'SRTM\tR1\tk2\tBPnd\tMSE\tLogLikelihood\tAIC\t%%Cov R1\t%%Cov k2\t%%Cov BPnd\n');
    for i=1:size(RoiTAC,2)
    ...
    function MakePlot(DataTAC,Name,ROIs)
        ...
        if isempty(fig_h)||New==1
            h=figure;
            set(h,'tag','CreateSRTMfig');
            ud.Data=DataTAC;
            ud.Name=Name;
            set(h,'UserData',ud);
            mh=uimenu(h,'Label','SRTM');
            seh1=uimenu(mh,'Label','Select ROIs','Accelerator','S','CallBack','CreateSRTM(''ShowROIs'');');
        else
        ...
        h=title(sprintf('SRTM fit: %s (%s)' ,Name.Label,datestr(clock,0)));
