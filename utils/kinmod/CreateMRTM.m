function [Err,Par,RoiName]=CreateMRTM(varargin)
%
% [Err,Par,RoiName]=CreateMRTM([tacFN,[StartStopTime,[SIFfn,Isotope]]])
%
%  Function that creates a txt file that can be imported into database with
%  the MRTM estimate. If missing frames in the middle of time series then mean
...
%
[TimeTAC,Weights,RefTAC,RoiTAC,Name,Dummy,DocTxt]=KinmodEvaluateInpArg('CreateMRTM',varargin{:});
RoiName=Name.Roi;
%
% Estimate parameters
%
...
for i=1:size(RoiTAC,2)
    fprintf('   MRTM: %s: R1=%5.3f, k2''=%5.3f, BPnd=%5.3f, WeightErr: %6.2e, MSE: %6.2e, FPE: %6.2e, LogLikelihood: %6.3e, AIC: %6.2e\n',...
...
pid=fopen([fn '.mrtm'],'w');
if pid==-1
    warning('MRTM: not possible to open file for saving MRTM results - probably write protected directory\n\n');
else
    %
    ...
    % Write data
    %
    fprintf(pid,'MRTM\tR1\tk2''\tBPnd\tMSE\tLogLikelihood\tAIC\t%%Cov R1\t%%Cov k2''\t%%Cov BPnd\n');
    for i=1:size(RoiTAC,2)
    ...
    function MakePlot(DataTAC,Name,ROIs)
        %
        fig_h=findobj('type','figure','tag','CreateMRTMfig');
        if ROIs==-1
            New=1;
            delete(fig_h);
        else
        ...
        if isempty(fig_h)||New==1
            h=figure;
            set(h,'tag','CreateMRTMfig');
            ud.Data=DataTAC;
            ud.Name=Name;
            set(h,'UserData',ud);
            mh=uimenu(h,'Label','MRTM');
            seh1=uimenu(mh,'Label','Select ROIs','Accelerator','S','CallBack','CreateMRTM(''ShowROIs'');');
        else
        ...
        h=title(sprintf('MRTM fit: %s (%s)' ,Name.Label,datestr(clock,0)));
