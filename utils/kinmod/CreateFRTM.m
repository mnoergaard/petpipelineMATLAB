function [Err,Par,RoiName]=CreateFRTM(varargin)
%
% [Err,Par,RoiName]=CreateFRTM([tacFN,[StartStopTime,[SIFfn,Isotope]]])
%
...
%
[TimeTAC,Weights,RefTAC,RoiTAC,Name,Dummy,DocTxt]=KinmodEvaluateInpArg('CreateFRTM',varargin{:});
RoiName=Name.Roi;

%
% Estimate parameters
%
[Err,Par,yest,Par_cov]=frtm_estm(TimeTAC,Weights,RefTAC,RoiTAC,Name);
%
% Calculate Chi-Square
%
%ChiSqr=sum((RoiTAC-yest).^2,1)./sum((RoiTAC-ones(size(RoiTAC,1),1)*mean(RoiTAC,1)).^2,1);
%ChiSqr2=sum((RoiTAC-yest).^2,1)./(std(RoiTAC)*38).^2;
%AIC=ChiSqr+2*3;
FPE=Err*(length(RefTAC)+3)/(length(RefTAC)-3);   % No of parameters in model is 3
%
...
%
fprintf('\n\5nData for FRTM model\n\n');
fprintf('   Data from %s\n\n',Name.Label');
for i=1:size(RoiTAC,2)
    fprintf('   FRTM: %s: R1=%5.3f, k2=%5.3f, k3=%5.3f, BPnd=%5.3f, LSQ: %6.2e, FPE: %6.2e\n',Name.Roi{i},Par{i},Err(i),FPE(i));
end
%
...
pid=fopen([fn '.frtm'],'w');
if pid==-1
    warning('FRTM: not possible to open file for saving FRTM results - probably write protected directory\n\n');
else
    %
    ...
    % Write data
    %
    fprintf(pid,'FRTM\tR1\tk2\tk3\tBPnd\tLSQ\tFPE\tStdErr R1\tStdErr k2\tStdErr k3\tStdErr BPnd\n');
    for i=1:size(RoiTAC,2)
        fprintf(pid,'%s\t%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\n',Name.Roi{i},Par{i},Err(i),FPE(i),sqrt(diag(Par_cov{i})));
    end
    fclose(pid);
end
    ...
    function MakePlot(DataTAC,Name,ROIs)
        %
        fig_h=findobj('type','figure','tag','CreateFRTMfig');
        if ROIs==-1
            New=1;
            delete(fig_h);
        else
        ...
        if isempty(fig_h)||New==1
            h=figure;
            set(h,'tag','CreateFRTMfig');
            ud.Data=DataTAC;
            ud.Name=Name;
            set(h,'UserData',ud);
            mh=uimenu(h,'Label','FRTM');
            seh1=uimenu(mh,'Label','Select ROIs','Accelerator','S','CallBack','CreateFRTM(''ShowROIs'');');
        else
        ...
        h=title(sprintf('FRTM fit: %s (%s)' ,Name.Label,datestr(clock,0)));
